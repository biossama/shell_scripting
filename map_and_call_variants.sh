...
refname=$(basename -s ".fna" $reference)

#This should be a glob of all desired viromes
...
bwa mem -t 46 $reference <(cat $reads) | samtools sort --threads 46 -o $refname.sorted.bam -

samtools index -@ 46 $refname.sorted.bam
...
#VARIANT CALLING
#I've decided to skip indels because they do not tell us nothing in terms of codon constraints


~/sambcf/bcftools-1.14/bcftools mpileup -a FORMAT/AD --threads 46 --skip-indels --max-depth $depth -f $reference $refname.sorted.bam |~/sambcf/bcftools-1.14/bcftools call --threads 46 -m --skip-variants indels -p 3000 -A > $refname.variants.bcf
